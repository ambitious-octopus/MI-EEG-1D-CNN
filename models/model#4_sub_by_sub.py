...
# XRes, desYRes = sm.fit_sample(X, desY)
# print('classes count')
# print ('before oversampling = {}'.format(desY.sum(axis=0)))
# print ('after oversampling = {}'.format(desYRes.sum(axis=0)))

exclude =  [38, 88, 89, 92, 100, 104]
subjects = [n for n in np.arange(1,2) if n not in exclude]
...
x_train, x_test, y_train, y_test = train_test_split(x_resh, y, stratify =y, test_size=0.30,  random_state=19)
...
print('classes count')
print ('before oversampling = {}'.format(y_test_arg.sum(axis=0)))
...
kernel_size = 2
drop_rate = 0.3 #0.2 good #0.4 local minimum
...
model.add(tf.keras.layers.Conv1D(filters=10, kernel_size=kernel_size, strides=1, activation='relu', padding= "valid", input_shape=(640, 2)))
model.add(tf.keras.layers.BatchNormalization())
model.add(tf.keras.layers.SpatialDropout1D(drop_rate))
model.add(tf.keras.layers.Conv1D(filters=10, kernel_size=5, activation='relu', strides=1, padding= "valid"))
model.add(tf.keras.layers.BatchNormalization())
# model.add(tf.keras.layers.SpatialDropout1D(drop_rate))
# model.add(tf.keras.layers.MaxPool1D(pool_size=3, strides=3))
model.add(tf.keras.layers.Conv1D(filters=10, kernel_size=kernel_size, activation='relu', strides=1, padding= "valid"))
model.add(tf.keras.layers.SpatialDropout1D(drop_rate))
model.add(tf.keras.layers.AvgPool1D(pool_size=3, strides=3))
...
model.summary()
hist = model.fit(x_train_resh, y_train, epochs=30, batch_size=32, validation_data=(x_test_resh, y_test))
