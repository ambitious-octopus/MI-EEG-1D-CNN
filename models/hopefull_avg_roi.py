...
# n_couple, n_epochs, t_point, channels
xs = x.reshape(9, 18331, 640, 2)
ys = y.reshape(9,18331)[0]

left_em = np.mean(xs[:, :, :, 0], axis=0)
right_em = np.mean(xs[:, :, :, 1], axis=0)

final_x = np.stack([left_em, right_em]).reshape(18331, 640, 2)

#Transform y to one-hot-encoding
y_one_hot  = Utils.to_one_hot(ys, by_sub=False)
#Reshape for scaling


reshaped_x = final_x.reshape(final_x.shape[0], final_x.shape[1] * final_x.shape[2])
...
model = HopefullNet()
modelPath = os.path.join(os.getcwd(),'anotherbestModel.h5')
...
with open(os.path.join(to_save, "avg_roi_hist.pkl"), "wb") as file:
...
Test model
"""
# path = "C:\\Users\\franc_pyl533c\\OneDrive\\Repository\\eeGNN\\models\\anotherbestModel.h5"
# model.load_weights(path)

model = tf.keras.models.load_model("D:\\hepefull_avg", custom_objects={"CustomModel": HopefullNet})
