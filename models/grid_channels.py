...
def run_grid(channels, name):
    ...
    model.fit(x_train, y_train, epochs=45, batch_size=10,
                     validation_data=(x_valid, y_valid), callbacks=callbacksList) #32
    #Save_model
    model.save("D:\\models\\" + name)
    return model.evaluate(x_test, y_test)

results = list()
for ch, na in zip(combination, comb_name):
    results.append(run_grid(ch, na))