...
TRAIN_PATH = "/home/kubasinska/datasets/eegbci/half_second"

with open(os.path.join(TRAIN_PATH, str(4) + ".pkl"), "rb") as file:
...
def gen_train():
    ...
    for i in range(1, max_index):
        with open(os.path.join(TRAIN_PATH, str(i)+".pkl"), "rb") as file:
            data = pickle.load(file)
        yield data[0].T, data[1]

x_train = tf.data.Dataset.from_generator(gen_train,
                                       output_types=(np.float64, np.float64),
                                       output_shapes=((80, 12),(5,)))

model = tf.keras.models.Sequential()
model.add(tf.keras.layers.Conv1D(filters=32, kernel_size=10, activation='relu', padding="same", input_shape=(80,12)))
# model.add(tf.keras.layers.TimeDistributed(tf.keras.layers.SpatialDropout1D(0.5)))
model.add(tf.keras.layers.BatchNormalization())
model.add(tf.keras.layers.Conv1D(filters=32, kernel_size=6, activation='relu'))
model.add(tf.keras.layers.BatchNormalization())
...
# fit network
model.summary()
model.fit(x = x_train.batch(25), epochs=70)